 ...
 * Updates Supabase `referral_events` and notifies the partner.
 */
async function handleOrderCancelled(order) {
  ...
  if (!referralTag) {
    logger.info('Cancelled order has no referral tag — skipping', { orderId: order.id });
    return;
  }
  ...
  if (profileErr || !profile) {
    logger.warn('Cancelled order has unknown referral tag', { referralTag, orderId: order.id });
    return;
  }
  ...
  // 4. Record cancellation event in Supabase
